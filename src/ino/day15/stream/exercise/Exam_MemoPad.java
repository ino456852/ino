...
public class Exam_MemoPad {
	...
	public static void main(String[] args) {
		...
		System.out.println("저장할 파일명 입력(확장자 제외) : ");	//가이드 메세지
		String fileName = sc.nextLine();							// 파일명 입력받음
		String dest = "src/ino/day15/stream/exercise/" + fileName + ".txt";
		// 입력받은 파일명으로 해당 경로 셋팅
		Writer writer = null;									// 출력 스트림 준비
		try {
			writer = new FileWriter(dest);			// 파일이 만들어지고 입력받을 준비 완료
			System.out.println("종료는 exit");
//			while(true) {}
			for(int i = 1;;i++) {					//무한반복 i는 1부터 시작하여 출력에서 이용
				System.out.print(i+" : ");			// 1 : , 2 : ... 출력
				String input = sc.nextLine();		// 저장할 데이터 입력
				if("exit".equals(input)) break;		// 널 포인터 익셉션 방지, exit 입력시 무한반복문 탈출
				writer.write(input + "\n");			// 해당 경로 파일에 저장
				writer.flush();						// 버퍼 지우기
			}
...
//		
//		try {
//			reader = new FileReader("src/ino/day15/stream/exercise/Exam_MemoPad.java");
//			int readCount;
